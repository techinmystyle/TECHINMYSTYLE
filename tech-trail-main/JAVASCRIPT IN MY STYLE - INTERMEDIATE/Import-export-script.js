...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Import / Export</code> in JavaScript are ES6 module features that allow you to share code between files using <code>export</code> to expose functions, objects, or values and <code>import</code> to consume them in other modules with syntax like <code>import {'{function}'} from './module'</code>. They're commonly used for code organization, creating reusable components, dependency management, building modular applications, or maintaining clean separation of concerns across different files and modules.</span>
    </p>
</div>
  `;
...
<head>
  ...
  <title>Import / Export in JavaScript</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>Import / Export in JavaScript</h1>
    <p>Click the button below to test ES6 import/export functionality.</p>
    <button id="runBtn">Run Demo</button>
    <pre id="logBox" class="output-box"></pre>
  </div>
...
.output-box {
  ...
  js: `// main.js (must be served via HTTP)
import { getTime, getGreeting } from './utils.js';
...
runBtn.addEventListener('click', () => {
  const time = getTime();
  const greeting = getGreeting("Basha");

  log("Current Time: " + time);
  log("Greeting: " + greeting);
});`,

  // utils.js (separate module file)
  module: `// utils.js
...
export function getGreeting(name) {
  return \`Hello, \${name}! This is a module-based greeting.\`;
}`
};
