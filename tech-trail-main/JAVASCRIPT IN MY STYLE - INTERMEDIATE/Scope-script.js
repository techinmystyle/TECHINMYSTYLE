...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Scope</code> in JavaScript determines the accessibility and visibility of variables, functions, and objects in different parts of code, including global scope, function scope, and block scope (with let/const), following lexical scoping rules where inner functions have access to outer function variables. It's commonly used for variable management, preventing naming conflicts, creating private variables, controlling data access, or organizing code structure where different parts of applications need controlled access to data and functionality.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    ...
    <p>Click the button to see examples of different scopes in JavaScript.</p>
    <button id="runBtn">Run Scope Demo</button>
...
runBtn.addEventListener('click', () => {
  ...
  function testFunction() {
    output += ' Inside function scope:\\n';
    var localVar = 'I am local';
    output += '  Accessing globalVar inside function: ' + globalVar + '\\n';
    output += '  localVar: ' + localVar + '\\n';

    if (true) {
      let blockVar = 'I am block scoped';
      output += '  Inside block scope: ' + blockVar + '\\n';
    }

    try {
      output += '  Trying to access blockVar outside block: ' + blockVar + '\\n';
    } catch (e) {
      output += '  Error accessing blockVar outside block: ' + e.message + '\\n';
    }
  }
  ...
  try {
    output += 'Accessing localVar outside function: ' + localVar + '\\n';
  } catch (e) {
    output += 'Error accessing localVar outside function: ' + e.message + '\\n';
  }
