// Combined script for info banner and Monaco editor functionality (eval version)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  infoBanner.innerHTML = `
    <div class="flex items-start">
        ...
        <p>
            <span>
              The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">eval()</code> function in JavaScript executes a string of JavaScript code at runtime. 
              <strong>Use with caution:</strong> <code>eval()</code> can execute arbitrary code, which can lead to security vulnerabilities and bugs. 
              Avoid using <code>eval()</code> unless absolutely necessary, and never use it with untrusted input.
            </span>
        </p>
    </div>
  `;
...
<head>
  ...
  <title>eval() Function Demo</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>eval() Function Demo</h1>
    <p>Enter a JavaScript expression to evaluate dynamically.</p>
    <input type="text" id="inputExpr" placeholder="e.g. 2 + 3 * 5" />
    <button id="evalBtn">Evaluate</button>
    <p><strong>Result:</strong></p>
    <pre id="result" class="output-box"></pre>
    <p class="warning">⚠️ Use <code>eval()</code> cautiously due to security risks.</p>
  </div>
...
.warning {
  ...
  js: `const inputExpr = document.getElementById('inputExpr');
...
evalBtn.addEventListener('click', () => {
  ...
  if (!expr) {
    resultBox.textContent = 'Please enter a JavaScript expression.';
    return;
  }
  try {
