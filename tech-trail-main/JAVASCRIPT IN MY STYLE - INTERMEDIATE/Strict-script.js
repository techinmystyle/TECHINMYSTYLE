...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Strict Mode</code> in JavaScript is enabled by adding <code>"use strict";</code> at the beginning of scripts or functions, enforcing stricter parsing and error handling by eliminating silent errors, preventing use of undeclared variables, and disabling certain problematic features. It's commonly used for writing safer code, catching common mistakes early, improving performance optimization, preventing accidental global variables, or ensuring modern JavaScript best practices in applications where code quality and reliability are important.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    <h1>JavaScript Strict Mode Demo</h1>
    <p>Click the button to see effects of strict mode on variable usage.</p>
    <button id="runBtn">Run Strict Mode Test</button>
...
runBtn.addEventListener('click', () => {
  ...
  // Non-strict mode example (will silently create global variable)
  output += 'Non-strict mode example:\\n';
  try {
    nonStrictFunction();
    output += 'nonStrictFunction executed without error. Global variable x created: ' + window.x + '\\n';
  } catch (err) {
    output += 'Error in nonStrictFunction: ' + err.message + '\\n';
  }

  // Strict mode example (should throw error)
  output += '\\nStrict mode example:\\n';
  try {
    strictFunction();
    output += 'strictFunction executed successfully.\\n';
  } catch (err) {
    output += 'Error in strictFunction: ' + err.message + '\\n';
  }

  outputLog.textContent = output;
});

// Functions demonstrating strict vs non-strict mode
function nonStrictFunction() {
  // Implicit global variable (not recommended)
  x = 10; // This creates a global variable in non-strict mode
}

function strictFunction() {
  'use strict';
  // This will throw ReferenceError because y is not declared
  y = 20; 
}`
};
