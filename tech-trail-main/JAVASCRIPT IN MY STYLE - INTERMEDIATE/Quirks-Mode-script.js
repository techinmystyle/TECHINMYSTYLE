...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Quirks Mode</code> in JavaScript refers to a browser rendering mode that emulates legacy behavior for backward compatibility with older websites, triggered by missing or invalid DOCTYPE declarations, affecting CSS layout, box model calculations, and some JavaScript behaviors. It's commonly encountered when debugging legacy websites, understanding cross-browser compatibility issues, fixing layout problems, or ensuring modern web standards compliance by using proper DOCTYPE declarations to trigger standards mode instead.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    <h1>Quirks Mode Demo</h1>
    <p>This demo explains Quirks Mode and Standards Mode in browsers.</p>

    <button id="checkModeBtn">Check Rendering Mode</button>
    <pre id="outputLog" class="output-box"></pre>

    <p>
      <strong>Note:</strong> Quirks Mode is triggered by certain DOCTYPE omissions or legacy HTML.
      This demo runs in Standards Mode due to the correct DOCTYPE.
    </p>
  </div>
...
.output-box {
  ...
  js: `const checkModeBtn = document.getElementById('checkModeBtn');
const outputLog = document.getElementById('outputLog');

checkModeBtn.addEventListener('click', () => {
  // Check if document is in Quirks Mode or Standards Mode
  let mode = (document.compatMode === 'CSS1Compat') ? 'Standards Mode' : 'Quirks Mode';
  
  outputLog.textContent = \`Rendering mode detected: \${mode}\\n\`;
  
  if (mode === 'Quirks Mode') {
    outputLog.textContent += 'The page is rendered using Quirks Mode, which emulates older browser behavior.\\n';
  } else {
    outputLog.textContent += 'The page is rendered using Standards Mode, which follows modern web standards.\\n';
  }
  
  outputLog.textContent += '\\nTry removing or modifying the DOCTYPE in the HTML file to see the effect on rendering mode.';
});`
};
