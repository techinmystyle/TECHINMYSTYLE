...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Ternary Operator</code> in JavaScript is a conditional operator using the syntax <code>condition ? valueIfTrue : valueIfFalse</code> that provides a concise way to write simple if-else statements as expressions that return values. It's commonly used for conditional assignments, inline conditional rendering, default value selection, simple conditional logic, or creating compact code where a full if-else statement would be verbose for straightforward true/false decisions.</span>
    </p>
</div>
  `;
