document.addEventListener("DOMContentLoaded", () => {
  ...
  infoBanner.innerHTML = `
    <div class="flex items-start">
        ...
        <p>
            <span>
              Event bubbling is a mechanism in JavaScript where an event triggered on an element propagates up through its parent elements in the DOM hierarchy. 
              <strong>Use with caution:</strong> Event bubbling can lead to unintended event handling if not properly managed. 
              Use <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">event.stopPropagation()</code> to prevent bubbling when needed.
            </span>
        </p>
    </div>
  `;
...
<body>
  <div class="event-demo-container" id="outer">
    <h1>Event Bubbling Demo</h1>
    <div class="inner" id="inner">
      <button class="event-btn" id="event-btn">Click Me!</button>
    </div>
...
function logEvent(elementId, event) {
  ...
  output.textContent += \`Event on \${elementId}: \${event.type} (Phase: \${event.eventPhase === 1 ? 'Capturing' : event.eventPhase === 2 ? 'Target' : 'Bubbling'})\n\`;
}

document.addEventListener("DOMContentLoaded", () => {
  const outer = document.getElementById("outer");
  const inner = document.getElementById("inner");
  const button = document.getElementById("event-btn");

  // Add event listeners for bubbling phase
  button.addEventListener("click", (e) => logEvent("button", e));
  inner.addEventListener("click", (e) => logEvent("inner div", e));
  outer.addEventListener("click", (e) => logEvent("outer div", e));

  console.log("Event bubbling demo is ready.");
});`
  };
