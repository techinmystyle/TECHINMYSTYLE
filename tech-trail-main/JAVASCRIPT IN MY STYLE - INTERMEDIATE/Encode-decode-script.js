...
document.addEventListener("DOMContentLoaded", () => {
  ...
  infoBanner.innerHTML = `
    <div class="flex items-start">
        ...
        <p>
            <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">URI Encode/Decode</code> functionality allows you to encode and decode Uniform Resource Identifiers (URIs). URI encoding ensures that URLs are correctly interpreted by replacing special characters with a format that can be transmitted over the Internet. Decoding reverses this process to retrieve the original characters.</span>
        </p>
    </div>
  `;
...
<head>
  ...
  <title>URI Encode/Decode Demo</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>URI Encode / Decode in JavaScript</h1>

    <div class="demo-section">
      <label for="inputText">Enter Text or URI:</label>
      <input type="text" id="inputText" placeholder="Type something to encode or decode" />
    </div>

    <div class="button-group">
      <button id="encodeBtn">Encode URI Component</button>
      <button id="decodeBtn">Decode URI Component</button>
    </div>
...
encodeBtn.addEventListener('click', () => {
  ...
  if (!text) {
    log('Please enter some text to encode.');
    return;
  }
  try {
    const encoded = encodeURIComponent(text);
    log(\`Encoded URI Component:\\n\${encoded}\`);
  } catch (err) {
    log('Error encoding URI component: ' + err.message);
  }
});
...
decodeBtn.addEventListener('click', () => {
  const text = inputText.value;
  if (!text) {
    log('Please enter some text to decode.');
    return;
  }
  try {
    const decoded = decodeURIComponent(text);
    log(\`Decoded URI Component:\\n\${decoded}\`);
  } catch (err) {
    log('Error decoding URI component: ' + err.message);
  }
});
`
};
