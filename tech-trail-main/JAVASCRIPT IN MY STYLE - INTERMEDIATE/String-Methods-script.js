...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">String Methods</code> in JavaScript are built-in functions for manipulating and working with strings, including methods like <code>slice()</code>, <code>substring()</code>, <code>split()</code>, <code>replace()</code>, <code>toLowerCase()</code>, <code>trim()</code>, and <code>indexOf()</code> for various text operations. They're commonly used for text processing, data validation, formatting user input, parsing data, search functionality, or manipulating strings for display, storage, and processing in web applications.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    ...
    <p>Click the button to see examples of common string methods.</p>
    <button id="runBtn">Run String Methods</button>
...
runBtn.addEventListener('click', () => {
  ...
  // charAt
  output += \`charAt(7): "\${str.charAt(7)}"\\n\`;

  // includes
  output += \`includes("Java"): \${str.includes("Java")}\\n\`;

  // indexOf
  output += \`indexOf("World"): \${str.indexOf("World")}\\n\`;

  // replace
  output += \`replace("World", "Universe"): "\${str.replace("World", "Universe")}"\\n\`;
  ...
  // split
  output += \`split(", "): [\${str.split(", ").join(", ")}]\\n\`;

  // substring
  output += \`substring(7, 17): "\${str.substring(7, 17)}"\\n\`;

  // trim
  const strWithSpaces = "   padded string   ";
  output += \`trim() on "   padded string   ": "\${strWithSpaces.trim()}"\\n\`;
