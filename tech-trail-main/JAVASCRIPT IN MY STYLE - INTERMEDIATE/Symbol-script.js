...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Symbol</code> in JavaScript is a primitive data type introduced in ES6 that creates unique, immutable identifiers that can be used as object property keys, ensuring no naming conflicts even if symbols have the same description. It's commonly used for creating private object properties, implementing well-known symbols like <code>Symbol.iterator</code>, avoiding property name collisions, creating unique constants, or building advanced metaprogramming patterns where unique identifiers are required.</span>
    </p>
</div>
  `;
...
<head>
  ...
  <title>Symbol in JavaScript</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>JavaScript Symbol Demo</h1>
    <p>Click the button to create a symbol and attach it to an object.</p>
    <button id="symbolBtn">Create Symbol</button>
...
const btn = document.getElementById('symbolBtn');

btn.addEventListener('click', () => {
  // Step 1: Create a Symbol
  const secretKey = Symbol('secret');

  // Step 2: Define an object with a symbol key
  const user = {
    name: "Alice",
    age: 25,
  ...
  // Step 3: Normal and Symbol keys
  const normalKeys = Object.keys(user);
  const symbolKeys = Object.getOwnPropertySymbols(user);

  // Step 4: Prepare output
  let output = '';
  output += 'User object (visible keys):\\n';
  output += JSON.stringify(user, null, 2) + '\\n\\n';
  output += 'Normal keys: ' + normalKeys.join(', ') + '\\n';
  output += 'Symbol keys found: ' + symbolKeys.length + '\\n';
  output += 'Accessing symbol value: ' + user[secretKey];
