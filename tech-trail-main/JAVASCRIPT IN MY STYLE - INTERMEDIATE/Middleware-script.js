...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Middleware</code> in JavaScript refers to functions that execute during the request-response cycle in frameworks like Express.js, or functions that intercept and process data flow in applications, typically having access to request, response, and next function parameters. It's commonly used for authentication, logging, error handling, request validation, CORS handling, or creating modular, reusable components that can be chained together to process requests or data transformations.</span>
    </p>
</div>
  `;
...
<head>
  ...
  <title>Middleware Simulation</title>
  <link rel="stylesheet" href="style.css" />
</head>
<body>
  <div class="container">
    <h1>Middleware in JavaScript</h1>
    <p>Click the button below to simulate a request pipeline with middleware functions.</p>
    <button id="startPipeline">Run Middleware Pipeline</button>
...
.output-box {
  ...
  js: `// Utility function to log to the output area
...
// Event listener
document.getElementById('startPipeline').addEventListener('click', () => {
  output.textContent = '';
  const ctx = { data: [] };
  const pipeline = compose([middleware1, middleware2, middleware3]);
  pipeline(ctx);
  log("\\nFinal Context Data: " + JSON.stringify(ctx.data));
});`
};
