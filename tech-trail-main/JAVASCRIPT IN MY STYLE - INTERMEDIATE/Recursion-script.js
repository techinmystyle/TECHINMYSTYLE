...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Recursion</code> in JavaScript is a programming technique where a function calls itself to solve a problem by breaking it down into smaller, similar subproblems, requiring a base case to prevent infinite loops and a recursive case that moves toward the base case. It's commonly used for tree traversal, mathematical calculations like factorials and Fibonacci sequences, parsing nested structures, implementing algorithms like quicksort and mergesort, or solving problems that have a naturally recursive structure.</span>
    </p>
</div>
  `;
...
const defaultCode = {
  ...
  js: '// Recursion Example - Factorial\nfunction factorial(n) {\n  if (n === 0 || n === 1) {\n    return 1;\n  }\n  return n * factorial(n - 1); // Recursive call\n}\n\nconst button = document.getElementById("calcBtn");\nconst output = document.getElementById("output");\n\nbutton.addEventListener("click", () => {\n  const number = 5;\n  const result = factorial(number);\n  output.textContent = `Factorial of ${number} is ${result}`;\n  console.log(`factorial(${number}) = ${result}`);\n});'
};
