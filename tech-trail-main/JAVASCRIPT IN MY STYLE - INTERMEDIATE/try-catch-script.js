...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="bg-orange-50 px-2 py-1 rounded mx-1 inline-block">try...catch</code> statement in JavaScript is an error handling mechanism that allows you to execute code in the try block and handle any exceptions that occur in the catch block, with an optional finally block that runs regardless of success or failure. It's commonly used for API error handling, input validation, preventing application crashes, graceful error recovery, or managing asynchronous operations where errors need to be caught and handled without breaking the entire application flow.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    <h1>try...catch in JavaScript</h1>
    <div class="demo-section">
      <label for="codeInput">Enter JavaScript Expression:</label>
      <input type="text" id="codeInput" placeholder="e.g., 10 / 2 or undefinedVariable + 1" />
    </div>
    <button id="runBtn">Run Expression</button>
    <h2>Output:</h2>
...
.output-box {
  ...
  js: `const codeInput = document.getElementById('codeInput');
...
runBtn.addEventListener('click', () => {
  ...
  if (!code) {
    log('Please enter an expression.');
    return;
  }

  try {
    const result = eval(code); // For demonstration only
    log('✅ Result: ' + result);
  } catch (error) {
