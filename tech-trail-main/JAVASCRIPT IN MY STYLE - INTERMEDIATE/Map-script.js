...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Map Object</code> in JavaScript is a built-in data structure that stores key-value pairs where keys can be of any type (objects, primitives, functions), providing methods like <code>set()</code>, <code>get()</code>, <code>has()</code>, and <code>delete()</code> with guaranteed insertion order. It's commonly used for caching, data mapping, lookup tables, maintaining insertion order, or scenarios requiring non-string keys that regular objects cannot handle effectively.</span>
    </p>
</div>
  `;
