...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Async / Await</code> syntax in JavaScript provides a cleaner way to handle asynchronous operations using async functions and await expressions. It's commonly used for API calls, file operations, database queries, or any asynchronous task where you want to write synchronous-looking code.</span>
    </p>
</div>
...
<head>
    ...
    <title>Async/Await Single Demo</title>
...
<body>
    <div class="container">
        ...
        <div class="demo-section">
            <p>Click the button to simulate fetching user data, then their posts, sequentially.</p>
            <button id="startDemoBtn">Start Async Demo</button>
...
async function startAsyncDemo() {
    disableButton(startDemoBtn);
    setStatus('Starting demo...', 'loading');
    clearLog();

    try {
        appendLog("Step 1: Fetching user data...");
        // Simulate fetching a user
        await delay(1500); // Simulate network latency
        const userResponse = await fetch('https://jsonplaceholder.typicode.com/users/1');
        if (!userResponse.ok) throw new Error(\`Failed to fetch user (Status: \${userResponse.status})\`);
        const userData = await userResponse.json();
        appendLog(\`User fetched: \<span class="math-inline">\{userData\.name\} \(\\</span>{userData.email})\`);

        appendLog("\\nStep 2: Fetching posts for the user...");
        // Simulate fetching posts for that user
        await delay(2000); // Simulate more network latency
        const postsResponse = await fetch(\`https://jsonplaceholder.typicode.com/posts?userId=\${userData.id}\`);
        if (!postsResponse.ok) throw new Error(\`Failed to fetch posts (Status: \${postsResponse.status})\`);
        const postsData = await postsResponse.json();
        appendLog(\`Found \<span class="math-inline">\{postsData\.length\} posts for \\</span>{userData.name}\`);
        if (postsData.length > 0) {
            appendLog(\`First post title: "\${postsData[0].title.substring(0, 50)}..."\`);
        }

        appendLog("\\nStep 3: All data processed!");
        setStatus('Demo completed successfully!', 'success');
    } catch (error) {
        appendLog(\`\\nError during demo: \${error.message}\`);
        setStatus('Demo failed!', 'error');
        console.error("Async Demo Error:", error);
    } finally {
        enableButton(startDemoBtn);
    }
}

// Attach event listener
document.addEventListener('DOMContentLoaded', () => {
    startDemoBtn.addEventListener('click', startAsyncDemo);
    setStatus('Ready to start', 'info'); // Initial status
});`
};
