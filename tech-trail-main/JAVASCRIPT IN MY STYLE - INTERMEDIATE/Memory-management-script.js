...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Memory Management</code> in JavaScript involves understanding how the JavaScript engine allocates, uses, and automatically frees memory through garbage collection, including concepts like heap allocation, reference counting, and preventing memory leaks. It's commonly used for performance optimization, debugging memory issues, preventing memory leaks, understanding application scalability, or building efficient applications that handle large datasets without consuming excessive memory resources.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    ...
    <div class="demo-section">
      <p>This demo shows how objects are created and memory is used in JS.</p>
      <button id="allocateBtn">Allocate Memory</button>
      <button id="releaseBtn">Release Memory</button>
      <pre id="logArea" class="output-box"></pre>
    </div>
  </div>
...
allocateBtn.addEventListener("click", () => {
  for (let i = 0; i < 1000; i++) {
    memoryStore.push({ data: new Array(1000).fill(Math.random()) });
  }
  log("Allocated memory: Added 1000 objects.");
});

// Simulate releasing memory
releaseBtn.addEventListener("click", () => {
  memoryStore = []; // Allow garbage collector to clean up
  log("Released memory: memoryStore cleared.");
});
...
// Info: JavaScript uses automatic garbage collection.
// Setting 'memoryStore = []' removes references, and unused memory is cleaned up.
`
};
