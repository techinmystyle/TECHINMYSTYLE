...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">JavaScript Engine (V8)</code> is Google's open-source JavaScript runtime that compiles JavaScript code to optimized machine code using just-in-time compilation, powering Chrome browser and Node.js with features like garbage collection, memory management, and performance optimization. It's commonly used for understanding JavaScript execution, performance optimization, Node.js development, or building high-performance applications that leverage V8's advanced compilation and optimization techniques.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    ...
    <div class="demo-section">
      <p>This demo simulates how the V8 engine parses, compiles, and executes JavaScript code using an interactive example.</p>
      <button id="runCodeBtn">Run JS Code</button>
      <pre id="executionLog" class="output-box"></pre>
    </div>
  </div>
...
.output-box {
  ...
  js: `const runCodeBtn = document.getElementById("runCodeBtn");
const executionLog = document.getElementById("executionLog");

function simulateV8Execution() {
  executionLog.textContent = ""; // Clear previous log

  log("Step 1: Parsing code...");
  setTimeout(() => {
    log("Step 2: Creating Abstract Syntax Tree (AST)...");
    setTimeout(() => {
      log("Step 3: Ignition bytecode compiler generates bytecode...");
      setTimeout(() => {
        log("Step 4: Code is interpreted and hot functions are optimized...");
        setTimeout(() => {
          log("Step 5: TurboFan compiles hot code to machine code for speed.");
          setTimeout(() => {
            log("✅ Execution completed via V8!");
          }, 500);
        }, 800);
      }, 800);
    }, 700);
...
function log(message) {
  ...
  executionLog.textContent += \`[\${time}] \${message}\\n\`;
  executionLog.scrollTop = executionLog.scrollHeight;
}

runCodeBtn.addEventListener("click", simulateV8Execution);`
};
