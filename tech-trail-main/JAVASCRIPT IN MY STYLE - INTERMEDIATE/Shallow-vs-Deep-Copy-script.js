...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Shallow vs Deep Copy</code> in JavaScript refers to two different ways of copying objects: shallow copy creates a new object but copies only the top-level properties (references to nested objects remain shared), while deep copy creates completely independent copies including all nested objects and arrays. They're commonly used for state management, data manipulation, preventing unintended mutations, implementing immutable patterns, or ensuring data integrity when working with complex nested data structures in applications.</span>
    </p>
</div>
  `;
