...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">Observer Pattern</code> in JavaScript is a behavioral design pattern where an object (subject) maintains a list of dependents (observers) and automatically notifies them of state changes, typically implemented using event listeners or custom notification systems. It's commonly used for event handling, state management, model-view architectures, reactive programming, or creating loosely coupled systems where components can communicate without direct dependencies on each other.</span>
    </p>
</div>
...
<body>
  <div class="observer-container">
    ...
    <button id="notifyBtn">Notify Observers</button>
    <ul id="logList"></ul>
  </div>
...
class Observer {
  ...
  update(data) {
    ...
    li.textContent = \`Observer \${this.id} received: \${data}\`;
...
subject.subscribe(new Observer(1));
subject.subscribe(new Observer(2));
subject.subscribe(new Observer(3));
