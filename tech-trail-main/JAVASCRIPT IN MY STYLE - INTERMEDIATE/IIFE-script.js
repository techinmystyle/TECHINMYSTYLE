...
<div class="flex items-start">
    ...
    <p>
        <span>The <code class="${randomColor.bg.replace('100', '50')} px-2 py-1 rounded mx-1 inline-block">IIFE (Immediately Invoked Function Expression)</code> in JavaScript is a function that is defined and executed immediately using syntax like <code>(function() {})()</code> or <code>(() => {})()</code>, creating a private scope that doesn't pollute the global namespace. It's commonly used for module patterns, avoiding variable conflicts, initialization code, creating closures, or encapsulating functionality that should run once without exposing internal variables to the global scope.</span>
    </p>
</div>
  `;
...
<body>
  <div class="container">
    <h1>IIFE Demo</h1>
    <p>This demo logs messages using Immediately Invoked Function Expressions.</p>
...
.output-box {
  ...
  js: `// IIFE Example - Immediately Invoked Function Expression
...
(function () {
  ...
  log("This message is from an IIFE!");

  // Example of IIFE returning a value
  const result = (function(a, b) {
    return a + b;
  })(10, 20);

  log("Result of IIFE with parameters: " + result);

  // IIFE with named function
  (function namedIIFE(name) {
    log(\`Hello from named IIFE, \${name}!\`);
  })("Basha");
